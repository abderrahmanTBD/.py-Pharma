...
columns=('Sl No', 'الاسم', 'النوع', 'الكمية', 'السعر', 'الأعراض', 'تاريخ إ.الصلاحية', 'الرف', 'المصنّع')

def open_win(): #OUVRE MENU ----------------------------------------------------------------------------MENU PRINCIPAL
    global apt, flag
    flag='apt'
    apt=Tk()
    apt.title("الصفحة الرئيسية")
    Label(apt, text="صيدلية كلية العلوم").grid(row=0,column=0,columnspan=3)
    Label(apt, text='*'*80).grid(row=1,column=0,columnspan=3)
    Label(apt, text='-'*80).grid(row=3,column=0,columnspan=3) 

    Label(apt, text="صيانة المخزون").grid(row=2,column=0)
    Button(apt,text='إضافة زبون', width=25, command=val_cus).grid(row=4,column=0)
    Button(apt,text='إضافة دواء', width=25,command=stock).grid(row=5,column=0)
    Button(apt,text='حذف دواء', width=25,command=delete_stock).grid(row=6,column=0)
    

    Label(apt, text="الوصول إلى المخزون").grid(row=2,column=1)
    Button(apt,text='تعديل',width=15, command=modify).grid(row=4,column=1)
    Button(apt,text='بحث', width=15, command=search).grid(row=5,column=1)
    Button(apt,text='تحقق من انتهاء الصلاحية', width=15, command=exp_date).grid(row=6,column=1)

    Label(apt, text="إدارة الأموال").grid(row=2,column=2)
    Button(apt,text="الدخل اليومي", width=20,command=show_rev).grid(row=5,column=2)
    Button(apt,text='الفواتير', width=20, command=billing).grid(row=4,column=2)

    Label(apt, text='-'*80).grid(row=12,column=0,columnspan=3)    
    Button(apt,text='الخروج',command=again).grid(row=13, column=2)
    apt.mainloop()
...
def delete_stock(): #OPENS DELETE WINDOW----------------------------------------------------DELETES A PARTICULAR STOCK ITEM
    ...
    flag='d'
    d=Tk()
    d.title("حذف دواء من المخزون")
    Label(d,text='اخل الدواء المراد حذفه:').grid(row=0,column=0)
    Label(d,text='',width=30,bg='white').grid(row=0,column=1)
    Label(d,text='الدواء').grid(row=2,column=0)
    Label(d,text='الثمن.  إ.الصلاحية. العدد                                ').grid(row=2,column=1)
    ren()
    b=Button(d,width=20,text='حذف',command=delt).grid(row=0,column=3)
    b=Button(d,width=20,text='القائمة الرئيسية',command=main_menu).grid(row=5,column=3)
    d.mainloop()
...
def modify():    # page de modification-----------------------------------------------------------------------MODIFIER
    ...
    Label(st, text='انقر على الدواء: ').grid(row=1, column=0)
    Label(st, text='ادخل القيمة المراد تغييرها').grid(row=2, column=0)
    att=Spinbox(st, values=col_n)
    att.grid(row=2, column=1)
    up=Entry(st)
    up.grid(row=2, column=2)
    Button(st,width=10,text='حفظ', command=save_mod).grid(row=2, column=4)
    Button(st,width=10,text='إعادة تعيين', command=res).grid(row=2, column=5)
    Button(st,width=15,text='عرض المعلومات', command=show_val).grid(row=1, column=4)
    Label(st, text='-'*120).grid(row=3,column=0,columnspan=6)
    Button(st,width=12,text='القائمة الرئيسية',command=main_menu).grid(row=5,column=5)
    st.mainloop()
...
def save_mod(): #enregistrer les donnees changees
    ...
    cur.execute(sql)
    c.commit()
    Label(st, text='تم الحفظ!').grid(row=5,column=4)
    
    
def stock():    #صفحة المخزون------------------------------------------------------------------------AJOUTER DANS LE STOCK
    global cur, c, columns, accept, flag, sto, apt
    apt.destroy()
    flag='sto'
    accept=['']*10
    sto=Tk()
    sto.title('إضافة دواء')
    Label(sto,text='ادخل معلومات الدواء الجديد').grid(row=0,column=0,columnspan=2)
    ...
    Button(sto,width=15,text='حفظ',command=submit).grid(row=12,column=1)
    Label(sto,text='-'*165).grid(row=13,column=0,columnspan=7)
    Button(sto,width=15,text='إعادة تعيين',command=reset).grid(row=12,column=0)
    Button(sto,width=15,text='تحديث المخزون',command=ref).grid(row=12,column=4)
    for i in range(1,6):
        Label(sto,text=columns[i]).grid(row=14,column=i-1)
    Label(sto,text='المصنّع   لرف    إ.الصلاحية                            ').grid(row=14,column=5)
    Button(sto,width=12,text='القائمة الرئيسية',command=main_menu).grid(row=12,column=5)
    ref()
    sto.mainloop()
...
def submit(): #pour submission de nouvelle entre de stock
    ...
    print (now-prev)
    top=Tk()
    Label(top,width=20, text='تم الحفظ!').pack()
    top.mainloop()
    main_menu()
...
def exp_date(): # ouvrire page d expiration -----------------------------------------------------------------------------expiration
    ...
    c.commit()
    exp=Tk()
    exp.title('تحقق من انتهاء الصلاحية')
    Label(exp,text='اليوم : '+str(now[2])+'/'+str(now[1])+'/'+str(now[0])).grid(row=0, column=0, columnspan=3)
    Label(exp,text='بيع الأدوية منتهية الصلاحية جريمة يعاقب عليها القانون').grid(row=1, column=0,columnspan=3)
    Label(exp,text='-'*80).grid(row=2, column=0,columnspan=3)
    s=Spinbox(exp,values=n)
    s.grid(row=3, column=0)
    Button(exp,text="تحقق من انتهاء الصلاحية", command=s_exp).grid(row=3, column=1)
    Label(exp,text='-'*80).grid(row=4, column=0,columnspan=3)
    if flags=='apt1':
        Button(exp,text='القائمة الرئيسية', command=main_cus).grid(row=5, column=2)
    else:
        Button(exp,width=20,text="التحقق من صلاحية كل الأدوية", command=exp_dt).grid(row=5, column=0)
        Button(exp,text='القائمة الرئيسية', command=main_menu).grid(row=5, column=2)
    exp.mainloop()
...
def s_exp():    # affiche la date dexpiration d un medicament entree
    ...
    for i in cur:
        if(i[1]==s.get()):
            ...
            if d1>d2:
                Label(exp, text='انتهت الصلاحية! يوم '+i[6]).grid(row=3, column=2)
                top=Tk()
                Label(top, text='انتهت الصلاحية!').pack()
            else:
                Label(exp, text=i[6]).grid(row=3, column=2)
    c.commit()

def exp_dt(): # affiche les medicament qui von expirer dans une semaine
    ...
    Label(exp,text='S.No'+'   '+'الاسم'+'     العدد.    '+'انتهاء الصلاحية').grid(row=6,column=0,columnspan=2)
    cur.execute("select * from med")
    for i in cur:
        s=i[6]
        ...
        if d1<d2<d3:
            ...
            top=Tk()
            Label(top,width=20, text=str(i[1])+' انتهت صلاحيته!').pack()
    c.commit()
...
def billing(): # cree une facture pour un client -------------------------------------------------------------system facturation
    ...
    Label(st,text='-'*48+'نــــــــظام الفـــــــــــــواتير'+'-'*49).grid(row=0,column=0,columnspan=7)
    Label(st,text='ادخال الاسم: ').grid(row=1,column=0)
    name1=Entry(st)
    name1.grid(row=1, column=1)
    Label(st,text='ادخل العنوان: ').grid(row=2,column=0)
    add=Entry(st)
    add.grid(row=2, column=1)
    Label(st,text="ادخل المعرّف (إذا توفر)").grid(row=3, column=0)
    vc_id=Entry(st)
    vc_id.grid(row=3, column=1)
    Button(st,text='تحقق من المعرّف', command=blue).grid(row=4, column=0)
    Label(st,text='-'*115).grid(row=6, column=0,columnspan=7)
    Label(st,text='اختر الدواء',width=25,relief='ridge').grid(row=7, column=0)
    Label(st,text='السعر       الكمية   الرف          ',width=25,relief='ridge').grid(row=7, column=1)
    Button(st,text='إضافة للفاتورة',width=15,command=append2bill).grid(row=8, column=6)
    Label(st,text='العدد',width=20,relief='ridge').grid(row=7, column=5)
    qtys=Entry(st)
    qtys.grid(row=8,column=5)
    refresh()
    Button(st,width=15,text='القائمة الرئيسية', command=main_menu).grid(row=1,column=6)
    Button(st,width=15,text='تحديث المخزون', command=refresh).grid(row=3,column=6)
    Button(st,width=15,text='اعادة تعيين الفاتورة', command=billing).grid(row=4,column=6)
    Button(st,width=15,text='طباعة الفاتورة', command=print_bill).grid(row=5,column=6)
    Button(st,width=15,text='تخزين الفاتورة', command=make_bill).grid(row=7,column=6)
...
def blue(): # verifier valeur id de client 
    ...
    for i in cur:
        if vc_id.get()!='' and int(vc_id.get())==i[2]:
            ...
            Label(st, text='زبون معروف').grid(row=4, column=1)
            t=1
            break
    c.commit()
...
def make_bill(): # faire facture
    ...
    m+="===============================================\n"
    m+="                                  No :%s\n\n" % det[5]
    m+="              صيدلية كلية العلوم والتقنيات    \n"
    m+="     جــــــــــــامعة انــــــــــــــــــواكشوك الـــــــــــــعصرية\n\n"
    m+="-----------------------------------------------\n"
    if t==1:
        m+="Name: %s\n" % named
        m+="Address: %s\n" % addd
    ...
    if t==1:
        ntotal=total*0.8
        m+='المجموع'+(' '*24)+(' '*(15-len(str(total)))) + str(total)+'\n'
        m+="التخفيض "+ (' '*(20-len(str(total-ntotal))))+'-'+str(total-ntotal)+'\n'
        m+="-----------------------------------------------\n"
        m+='المجموع'+(' '*24)+(' '*(12-len(str(ntotal)))) +'MRU '+ str(ntotal)+'\n'
        det[3]=str(ntotal)
    else:
    ...
    m+="-----------------------------------------------\n\n"
    m+=" __________________________: توقــــيع الصيدلاني\n"
    m+="===============================================\n"
    m.encode()
    print (m)
    p=time.localtime()
...
def show_rev(): # ouvre page de revenue ----------------------------------------------------------------------- TOTAL REVENUE
    ...
    for i in cur:
        if i[4]==today:
            total+=float(i[3])
    print (total)
    Label(rev,width=22,text='أوقية'+str(total)+':الدخل الاجمالي', bg='black',fg='white').grid(row=1,column=0)
    cx=0
    ...
    for i in cur:
        if i[4]==today:
            cx+=1
            lb1.insert(cx,'رقم الفاتورة.: '+str(i[5])+'    : أوقية '+str(i[3]))
    c.commit()
    Button(rev,text='القائمة الرئيسية',command=main_menu).grid(row=15,column=0)
    rev.mainloop()
...
def search():   #page chercher un medicament et symptome --------------------------------- chercher medicament
    ...
    st=Tk()
    st.title('بحث')
    Label(st, text=' بحث عن دواء ').grid(row=0, column=0,columnspan=3)
    Label(st, text='~'*40).grid(row=1, column=0,columnspan=3)
    Label(st, text='الأعراض').grid(row=3, column=0)
    sym=Spinbox(st,values=symp)
    sym.grid(row=3, column=1)
    Button(st,width=15, text='بحث', command=search_med).grid(row=3, column=2)
    Label(st, text='-'*70).grid(row=4, column=0,columnspan=3)
    if flags=='apt1':
        Button(st,width=15, text='القائمة الرئيسية', command=main_cus).grid(row=6, column=2)
    else:
        Button(st,width=15, text='القائمة الرئيسية', command=main_menu).grid(row=6, column=2)
    st.mainloop()
...
def search_med():
    ...
    for i in cur:
        if i[5]==sym.get():
            y.append(str(i[0])+'. '+str(i[1])+'  أوقية '+str(i[4])+'    الرف : '+str(i[7])+'    المصنّع : '+str(i[8]))
            x=x+1
    top=Tk()
    for i in range(len(y)):
        Label(top,text=y[i]).grid(row=i, column=0)
    Button(top,text='تم',command=top.destroy).grid(row=5, column=0)
    c.commit()
    top.mainloop()
...
def val_cus():  #nouveau utilisateur-----------------------------------------------------------nouveau utilisateur
    ...
    flag='val'
    val=Tk()
    val.title("إضافة زبون")
    Label(val,text="أدخل معلومات الزبون الجديد").grid(row=0,column=0,columnspan=3)
    Label(val,text="-"*60).grid(row=1,column=0,columnspan=3)
    Label(val,text="الأسم: ").grid(row=2,column=0)
    name_vc=Entry(val)
    name_vc.grid(row=2, column=1)
    Label(val,text="العنوان: ").grid(row=3,column=0)
    add_vc=Entry(val)
    add_vc.grid(row=3, column=1)
    Label(val,text="المعرّف: ").grid(row=4,column=0)
    vc_id=Entry(val)
    vc_id.grid(row=4, column=1)
    Button(val,text='حفظ',command=val_get).grid(row=5, column=1)
    Button(val,text='القائمة الرئيسية',command=main_menu).grid(row=5, column=2)
...
def again():    #page connection #login#-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    root.title('صيدلية كلية العلوم')
    Label(root,text='صيدلية كلية العلوم').grid(row=0,column=0,columnspan=5)
    Label(root,text="جامعة انواكشوط العصرية").grid(row=1,column=0,columnspan=5)
    Label(root,text='-------------------------------------------------------').grid(row=2,column=0,columnspan=5)
    Label(root, text='اسم المستخدم').grid(row=3, column=0)
    Label(root, text='u',image=usr).grid(row=3, column=1)
    un=Entry(root,width=10)
    un.grid(row=3, column=2)
    Label(root, text='كلمة السر').grid(row=4, column=0)
    Label(root, text='p',image=pas).grid(row=4, column=1)
    pwd=Entry(root,width=10, show='*')
    pwd.grid(row=4, column=2)
    Button(root,width=6,text='دخول',command=check).grid(row=5, column=1)
    Button(root,width=6,text='إغلاق',command=root.destroy).grid(row=5, column=2)
    root.mainloop()
...
def open_cus(): #OUVRE MENU PRINCIPALE----------------------------------------------------------------------------MENU PRINCIPALE
    ...
    flags='apt1'
    apt=Tk()
    apt.title("صيدلية كلية العلوم")
    Label(apt, text="PYTHON PHARMA").grid(row=0,column=0)
    Label(apt, text='*'*40).grid(row=1,column=0)
    Label(apt, text='*  مرحبـــــــا  *').grid(row=2,column=0)
    Label(apt, text='-'*40).grid(row=3,column=0)
    Label(apt, text="خدمة الزبناء").grid(row=4,column=0)
    Label(apt, text='-'*40).grid(row=5,column=0)
    Button(apt,text='بحث', width=15, command=search).grid(row=6,column=0)
    Button(apt,text='تحقق من انتهاء الصلاحية', width=15, command=exp_date).grid(row=7,column=0)
